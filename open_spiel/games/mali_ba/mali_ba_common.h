...
// Constants - open_spiel requires an 'Action' to be an integer.
    ...
    constexpr Action kChanceSetupAction = 900; // at the time of writing, this is higher than any possible action ID generated by the current action encoding scheme. It is used as a special flag to indicate chance setup actions.
    constexpr int kUnlimitedPosts = -1;

    // A safe upper bound for board size. Must be defined before action offsets.
    constexpr int kMaxHexes = 100; 

    // ==============================================================================
    // THE NEW COMPACT ACTION SPACE
    // ==============================================================================
    ...
    // Select hex to place a token during Setup (Size: kMaxHexes)
    constexpr int kHexSelectionBase = kMancalaStartBase + kMaxHexes;
    // Select hex to upgrade a post to a center (Size: kMaxHexes)
    constexpr int kUpgradeBase = kHexSelectionBase + kMaxHexes; 
    ...
    // Total maximum actions is vastly smaller (~424)
    ...
    // Other constants
    inline constexpr int MaxGameLength() { return 460; }  // Play-phase moves only; setup moves added by Mali_BaGame::MaxGameLength()
    ...
    struct GameRules {
      ...
      int end_game_cond_num_rare_goods = 5;     // 5 different rare goods triggers end
      ...
      // Points for sets of unique resources (ALL goods: common + rare combined).
      // Key = number of distinct resource types held; value = pts for that set count.
      std::map<int, int> score_unique_goods = {
        {1,1}, {2,3}, {3,6}, {4,11}, {5,19}, {6,30}, {7,45},
        {8,60}, {9,75}, {10,90}, {11,110}
      };
      int score_unique_goods_bonus = 20; // additional pts for each set beyond 11
      // Map for regions crossed points (per route)
      std::map<int, int> score_regions_crossed = {
        {1,4}, {2,8}, {3,12}, {4,17}, {5,23}, {6,30}
      };
      // Points for region control (1st, 2nd, 3rd place by center count)
    ...
    struct TrainingParameters {
      ...
      // Maximum play-phase moves before the game times out (excluding setup moves)
      int max_play_moves = 460;
      // Extra moves granted to near-win games that reach max_play_moves
